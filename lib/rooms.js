...
function createHub(opt) {
  ...
  function startGame(p) {
    ...
    const now = now0();
    const state = Rules.create(players, { seed, pace: room.pace, autoStartMs: CONST.AUTO_START_MS, now });
  ...
  function endGame(room, now) {
    ...
    sys(room, w.name + ' 把牌出完，獲勝！按準備好可以再來一局');
    room.game = null;
  ...
  function listRooms() {
    return [...rooms.values()].map(r => ({
      ...
      specs: r.specs.length, playing: !!r.game, pace: r.pace,
  ...
  function roomView(room, pid, now) {
    ...
    return {
      id: room.id, name: room.name, max: room.max, pace: room.pace, aiDiff: room.aiDiff,
